...
async function testEndpoint() {
  ...
  try {
    ...
    const res1 = await fetch('http://localhost:5000/api/v1/patients?q=&page=1&limit=50&visitType=&departmentId=&doctorId=&state=&city=&startDate=&endDate=&sortBy=newest', { headers });
    ...
    const res2 = await fetch('http://localhost:5000/api/v1/patients?q=Saravanan', { headers });
    ...
    const res3 = await fetch('http://localhost:5000/api/v1/patients?q=Saravanan Govindasamy', { headers });
    ...
    const res4 = await fetch('http://localhost:5000/api/v1/patients?state=Tamil Nadu&city=Chennai', { headers });
