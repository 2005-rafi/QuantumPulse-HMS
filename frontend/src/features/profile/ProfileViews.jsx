...
export const PERMISSION_MODULES = [
  ...
  },
  {
    ...
    codes: [
      ...
      { code: 'MEDICINE_PRICE_MANAGE', label: 'Manage Medicine Prices', description: 'Update pharmaceutical catalog rates' },
    ],
  },
  {
    module: 'Billing, Cashier & Tariff',
    icon: 'payments',
    description: 'Invoice generation, charges management, and payment receipts',
    codes: [
      { code: 'BILL_VIEW', label: 'View Invoices & Bills', description: 'Inspect patient bills and payment ledgers' },
      ...
      { code: 'TARIFF_VIEW', label: 'View Tariff Schedules', description: 'Inspect standard service tariffs' },
      { code: 'TARIFF_MANAGE', label: 'Manage Tariffs & Rules', description: 'Configure hospital service price rules' },
    ],
  },
  {
...
export const ProfileOverviewView = () => {
  const { staffDetails, roleName, accountStatus, formattedDate, formattedDateTime } = useOutletContext();
...
export const ProfileEmploymentView = () => {
  const { staffDetails, formattedDate } = useOutletContext();
  if (!staffDetails) return null;
...
export const ProfileAccessView = () => {
  const { staffDetails, roleName, userPermissionCodes } = useOutletContext();
  if (!staffDetails) return null;
...
export const ProfileCredentialsView = () => {
  const { staffDetails, roleName, formattedDate } = useOutletContext();
  if (!staffDetails) return null;
...
/* ─── TAB 5: DOCUMENTS ─── */
export const ProfileDocumentsView = () => {
  const { staffDetails, formattedDate, handleDownload } = useOutletContext();
  if (!staffDetails) return null;
...
export const ProfileHistoryView = () => {
  const { positionHistory, formattedDate } = useOutletContext();
...
export const ProfileSettingsView = () => {
  const { staffDetails, roleName, user } = useOutletContext();
  const { isListView: isPatientList, isCardView: isPatientCards, setLayout: setPatientLayout } = usePatientLayoutPreference();
      ...
      <div className="profile-settings-grid">
        ...
        {(roleName === 'Administrator' || roleName === 'Pharmacy' || user?.role === 'Administrator' || user?.role === 'Pharmacy') && (
          <div className="profile-md3-card" style={{ gridColumn: '1 / -1' }}>
            ...
            <div className="profile-layout-pref-body" style={{ padding: '16px' }}>
              <p style={{ margin: '0 0 14px 0', fontSize: '13px', color: 'var(--md-sys-color-on-surface-variant, #4a4539)' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(260px, 1fr))', gap: '14px' }}>
                ...
                >
                  <div style={{
                    ...
                    <span className="material-symbols-rounded" style={{ fontSize: '20px' }}>grid_view</span>
                  </div>
                  <div style={{ flex: 1 }}>
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '3px' }}>
                      <strong style={{ fontSize: '13.5px', color: 'var(--md-sys-color-on-surface, #1d1b16)' }}>Card Grid View</strong>
                      {isPatientCards && (
                        <span style={{
                          fontSize: '10.5px',
                          fontWeight: '700',
                    ...
                    <p style={{ margin: 0, fontSize: '12px', color: 'var(--md-sys-color-on-surface-variant, #797667)', lineHeight: 1.4 }}>
                ...
                >
                  <div style={{
                    ...
                    <span className="material-symbols-rounded" style={{ fontSize: '20px' }}>view_list</span>
                  </div>
                  <div style={{ flex: 1 }}>
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '3px' }}>
                      <strong style={{ fontSize: '13.5px', color: 'var(--md-sys-color-on-surface, #1d1b16)' }}>Tabular List View</strong>
                      {isPatientList && (
                        <span style={{
                          fontSize: '10.5px',
                          fontWeight: '700',
