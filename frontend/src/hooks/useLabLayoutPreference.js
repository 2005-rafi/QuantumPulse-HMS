...
const STORAGE_KEY = 'careone_lab_layout_preference';
const PREFERENCE_EVENT = 'careone_lab_layout_change';

/**
 * Custom Hook for managing the Laboratory Directory layout preference ('cards' | 'list').
 * Follows Single Responsibility Principle (SRP) by isolating layout state & persistence.
 */
export const useLabLayoutPreference = () => {
  ...
  const setLayout = useCallback((newLayout) => {
    ...
    try {
      ...
      console.warn('[useLabLayoutPreference] Could not persist preference', err);
    }
  }, []);
...
export default useLabLayoutPreference;
