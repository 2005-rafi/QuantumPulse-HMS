...
const STORAGE_KEY = 'careone_audit_layout_preference';
const PREFERENCE_EVENT = 'careone_audit_layout_change';

/**
 * Custom Hook for managing the Audit Logs layout preference ('cards' | 'list').
 * Follows Single Responsibility Principle (SRP) by isolating layout state & persistence.
 */
export const useAuditLayoutPreference = () => {
  ...
  const setLayout = useCallback((newLayout) => {
    ...
    try {
      ...
      console.warn('[useAuditLayoutPreference] Could not persist preference', err);
    }
  }, []);
...
export default useAuditLayoutPreference;
