...
const STORAGE_KEY = 'careone_staff_layout_preference';
const PREFERENCE_EVENT = 'careone_staff_layout_change';

/**
 * Custom Hook for managing the Staff Directory layout preference ('cards' | 'list').
 * Follows Single Responsibility Principle (SRP) by isolating layout state & persistence.
 */
export const useStaffLayoutPreference = () => {
  ...
  const setLayout = useCallback((newLayout) => {
    ...
    try {
      ...
      console.warn('[useStaffLayoutPreference] Could not persist preference', err);
    }
  }, []);
...
export default useStaffLayoutPreference;
